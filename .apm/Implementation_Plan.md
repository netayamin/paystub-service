...
**Last Modification:** Product A feed path: `build_feed` drops weak just-opened-only cards, applies eligibility rank multipliers to priority/ticker; push job filters on `push_notification_allowed`; pytest config + `tests/test_eligibility.py`, `tests/test_feed_eligibility.py`. Phases 7–9 remain plan-gated.
