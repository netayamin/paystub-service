...
**Last Modification:** Phase 7 slice: `GET /chat/watches/follows/status` + `activity` (effective notify list, last drop from `drop_events`, timeline from `user_notifications`); push job ordering + optional “Rare opening” title (`push_scoring.py`); iOS Saved tab loads status/activity + subtitles. Phases 8–9 still gated.
